    ...
    <div class="bg-surface-200 border border-surface-300 rounded-xl p-6 mb-4">
      ...
      {:else}
        <form on:submit|preventDefault={saveProfile} class="space-y-4">
          <div>
            <label for="profile-business-name" class="block text-sm font-medium mb-1 text-surface-950">Business Name</label>
            <input id="profile-business-name" bind:value={form.business_name} required class="w-full px-3 py-2 bg-surface-100 border border-surface-300 rounded-lg text-surface-950" />
          </div>
          <div>
            <label for="profile-phone" class="block text-sm font-medium mb-1 text-surface-950">Contact Phone</label>
            <input id="profile-phone" bind:value={form.contact_phone} type="tel" class="w-full px-3 py-2 bg-surface-100 border border-surface-300 rounded-lg text-surface-950" />
          </div>
          <div>
            <label for="profile-email" class="block text-sm font-medium mb-1 text-surface-950">Contact Email</label>
            <input id="profile-email" bind:value={form.contact_email} type="email" class="w-full px-3 py-2 bg-surface-100 border border-surface-300 rounded-lg text-surface-950" />
          </div>
          <div>
            <label for="profile-visibility" class="block text-sm font-medium mb-1 text-surface-950">Contact Visibility</label>
            <select id="profile-visibility" bind:value={form.contact_visibility} class="w-full px-3 py-2 bg-surface-100 border border-surface-300 rounded-lg text-surface-950">
